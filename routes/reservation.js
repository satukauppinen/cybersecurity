...
export async function registerReservation(c) {
    ...
    try {
        ...
        await client.queryArray(query, [pseudonym, resourceId, startTime, endTime]);
...
export async function getReservations(c) {
    const query = `SELECT resource_id, start_time, end_time FROM abc123_public_reservations`;
    try {
